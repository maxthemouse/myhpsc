...
real(kind=8) function simpson(f, a, b, n)
    ...
    ! Local variables:
    integer :: j
    real(kind=8) :: h, simp_sum, xj, xc

    h = (b-a)/(n-1)
    simp_sum = f(a) + f(b)  ! endpoint contributions
    
    !$omp parallel do private(xj) reduction(+ : simp_sum) 
    do j=2,n-1
        xj = a + (j-1)*h
        simp_sum = simp_sum + 2.0d0 * f(xj)
        enddo

    !$omp parallel do private(xc) reduction(+ : simp_sum) 
    do j=1,n-1
        xc = a + (j-0.5d0)*h
        simp_sum = simp_sum + 4.0d0 * f(xc)
        enddo

    simpson = (h / 6.0d0) * simp_sum
